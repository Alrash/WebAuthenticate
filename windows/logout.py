...
urlLogout = "http://a.nuist.edu.cn/index.php/index/logout"
urlInit =  "http://a.nuist.edu.cn/index.php/index/init"
...
def show_status_logout(status, time):
    print("*****************************************************")
    try:
        print(status['info'])
        print("本次在线时长为: " + time)
...
def show_status():
    ...
    if status['status'] != 0:
        try:
            time = show_time(status['logout_timer'])
        except KeyError:
            print("认证页面已更换api，可以联系作者更新程序了-_-|||")
            return
        status = get_status_with_json(urlLogout)
        show_status_logout(status, time)
    else:
        show_status_init(status)
    return
